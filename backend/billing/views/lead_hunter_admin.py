...
class LeadHunterDeviceOtpEmailView(APIView):
    permission_classes = []

    def post(self, request):
        secret = getattr(settings, "MAIL_FLOW_LEADHUNT_RELAY_SECRET", getattr(settings, "MAIL_FLOW_OTP_RELAY_SECRET", ""))
        provided_secret = request.headers.get("X-Mail-Flow-Secret") or request.META.get("HTTP_X_MAIL_FLOW_SECRET", "")
        if not secret or not provided_secret or not hmac.compare_digest(secret, provided_secret):
            return Response({"ok": False, "detail": "Unauthorized."}, status=status.HTTP_401_UNAUTHORIZED)

        email = (request.data.get("email") or "").strip().lower()
        code = (request.data.get("code") or "").strip()
        if not email or "@" not in email:
            return Response({"ok": False, "detail": "A valid email address is required."}, status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response({"ok": True, "detail": "Email accepted."}, status=status.HTTP_202_ACCEPTED)
