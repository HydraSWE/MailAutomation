...
export default function LandingPricing({ plans, loadingPlans, error }) {
  ...
  const premiumPlusPlan =
    plans.find((plan) => plan.slug === "premium-plus") ||
  ...
  const dynamicAddonPrices = customPlan?.addon_prices || premiumPlusPlan?.addon_prices || null;
          ...
          : (
            <>
              {visibleFixedPlans.map((plan) => {
                ...
                const badgeLabel = plan.badge_text || "Recommended";
                const isPlanFree = Boolean(
                  plan.is_free ||
                    (plan.price_bdt === 0 && !plan.original_price_bdt) ||
                    plan.slug === "free"
                );
                const buttonLabel =
                  plan.button_text ||
                  (isPlanFree ? "Create free account" : "Subscribe with USDT");
                const customFeatures =
                  Array.isArray(plan.features_list) && plan.features_list.length > 0
                    ? plan.features_list
                    : null;
                  ...
                  >
                    ...
                    <div>
                      ...
                      <ul className="space-y-3.5 mt-5 text-xs text-slate-300">
                        {customFeatures ? (
                          customFeatures.map((featureItem, index) => (
                    ...
                    <button
                      ...
                      className={`mt-8 w-full rounded-xl py-3 px-4 text-xs font-bold flex items-center justify-center gap-1.5 transition-all duration-200 active:scale-95 ${
                        featured
              ...
              <CustomPlanCard
                basePlan={premiumPlusPlan}
                customPlan={customPlan}
                addonPrices={dynamicAddonPrices}
              />
            </>
          )}
      </div>
