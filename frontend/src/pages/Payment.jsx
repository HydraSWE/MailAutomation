...
export default function Payment() {
  ...
  if (invoice.status === "paid") {
    const isCustomQuote = Boolean(invoice.snapshot_limits?.custom_quote);
    return (
      <PageCenter>
        ...
        <p>
          {isCustomQuote
            ? "Your custom plan payment has been verified on-chain. Check your email for your activation link."
            : `Your ${invoice.plan.name} workspace is active for the next 30 days.`}
        </p>
        <Link
          to={isCustomQuote ? "/login" : "/login?created=1"}
          className="px-6 py-3 rounded-md bg-emerald-400 text-emerald-950 font-black"
        >
          {isCustomQuote ? "Go to Login" : "Sign in to your account"}
        </Link>
        {invoice.explorer_url && (
          <a href={invoice.explorer_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-sm text-slate-500">
  ...
  if (invoice.status === "manual_review" || invoice.status === "review_required") {
    return (
      <PageCenter>
        <Clock3 className="w-14 h-14 text-amber-300" />
        <h1 className="text-3xl font-black">Payment Under Review</h1>
        <p className="max-w-lg text-center">
          {invoice.verification_error || "We recorded your blockchain transfer and flagged it for owner review. You will receive an email once verified."}
        </p>
        {invoice.explorer_url && (
          <a href={invoice.explorer_url} target="_blank" rel="noreferrer" className="text-indigo-300">
  ...
  if (["cancelled", "replaced", "payment_rejected"].includes(invoice.status)) {
    return (
      <PageCenter>
        <XCircle className="w-14 h-14 text-slate-500" />
        <h1 className="text-3xl font-black">Invoice {invoice.status.replace("_", " ")}</h1>
        <Link to={invoice.replaced_by ? `/payment/${invoice.replaced_by}` : "/#pricing"} className="text-indigo-300">
          Continue
        </Link>
      </PageCenter>
    );
  ...
  const customLimits = invoice.snapshot_limits?.custom_plan || invoice.snapshot_limits?.custom_quote ? invoice.snapshot_limits : null;
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 px-4 py-8 lg:py-14">
      <div className="max-w-3xl mx-auto">
        <Link to="/#pricing" className="inline-flex items-center gap-2 text-sm text-slate-400">
        ...
        <div className="mt-8 border border-white/10 bg-slate-900/65 overflow-hidden shadow-2xl rounded-md">
          <div className="p-7 sm:p-9 border-b border-white/7 flex flex-col sm:flex-row gap-5 sm:items-center justify-between">
            <div>
              ...
              <div className="flex items-center gap-3 mt-2 flex-wrap">
                ...
                {invoice.plan?.discount_percent > 0 && (
                  <span className="px-2 py-0.5 rounded-full bg-emerald-400/15 text-emerald-400 text-[10px] font-bold uppercase tracking-wider">
                    {invoice.plan.discount_percent}% Discount applied
                  </span>
                )}
              </div>
          ...
          <div className="p-7 sm:p-9 space-y-7">
            {customLimits && (
              <div className="rounded-md border border-cyan-400/20 bg-cyan-400/5 p-5 text-sm text-slate-200">
                <h2 className="font-bold text-cyan-200">
                  {customLimits.quote_number ? `Approved Quote #${customLimits.quote_number} Limits` : "Custom plan details"}
                </h2>
                <div className="mt-3 grid gap-2 sm:grid-cols-2 text-xs text-slate-400">
                  {[
                    `${Number(customLimits.email_limit).toLocaleString()} emails / 30 days`,
                    `${Number(customLimits.max_admins).toLocaleString()} admins`,
            ...
            <div>
              <p className="text-xs font-bold text-slate-400 mb-2">Receiving address</p>
              <div className="bg-slate-950 border border-slate-700 p-4 flex items-center gap-3 rounded-md">
                <Wallet className="w-5 h-5 text-indigo-400 shrink-0" />
                <code className="text-xs sm:text-sm break-all flex-1">{invoice.receiving_address}</code>
                <button title="Copy wallet address" onClick={() => copy(invoice.receiving_address, "wallet")}>
            ...
            <form onSubmit={verify}>
              ...
              {(error || invoice.verification_error) && <div className="mt-3 text-sm text-rose-300">{error || invoice.verification_error}</div>}
              <button disabled={Boolean(action)} className="mt-4 w-full py-3.5 rounded-md bg-indigo-500 hover:bg-indigo-400 font-bold disabled:opacity-50 flex justify-center items-center gap-2">
                {action === "verify" ? <><Loader2 className="w-4 h-4 animate-spin" /> Checking blockchain…</> : "Verify payment"}
              </button>
            </form>
            {expired && (
              <form onSubmit={replace} className="border-t border-white/10 pt-6">
                <h2 className="font-bold">Create a fresh quote</h2>
                <p className="text-sm text-slate-500 mt-1">Enter your account password again to securely carry registration into a new invoice.</p>
                <input type="password" minLength="8" required value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Account password" className="mt-4 w-full bg-slate-950 border border-slate-700 px-4 py-3 rounded-md" />
                <button disabled={Boolean(action)} className="mt-3 w-full border border-slate-600 py-3 rounded-md font-bold">Create replacement invoice</button>
              </form>
            )}
      ...
      {blocker.state === "blocked" && (
        <div className="fixed inset-0 z-50 bg-black/70 grid place-items-center p-4">
          <div className="w-full max-w-md bg-slate-900 border border-slate-700 p-6 rounded-md">
            <h2 className="text-xl font-black">Leave this payment?</h2>
            <p className="text-sm text-slate-400 mt-2">Use the secure link from your email if you need to resume later.</p>
            <div className="flex gap-3 mt-6">
              <button onClick={() => blocker.reset()} className="flex-1 bg-indigo-500 py-3 rounded-md font-bold">Continue payment</button>
              <button onClick={() => blocker.proceed()} className="flex-1 border border-slate-600 py-3 rounded-md">Leave</button>
            </div>
          </div>
        </div>
      )}
  ...
  return (
    <form onSubmit={submit} className="w-full max-w-md">
      <div className="flex gap-2">
        <input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="Your checkout email" className="min-w-0 flex-1 bg-slate-950 border border-slate-700 px-4 py-3 rounded-md" />
        <button disabled={busy} className="px-4 bg-indigo-500 rounded-md font-bold">Email link</button>
      </div>
...
function PageCenter({ children }) {
  return (
    <div className="min-h-screen bg-[#070b14] text-slate-100 grid place-items-center p-5">
      <div className="max-w-xl w-full text-center flex flex-col items-center gap-5 border border-white/10 bg-slate-900/65 p-8 rounded-md">{children}</div>
    </div>
  );
}
