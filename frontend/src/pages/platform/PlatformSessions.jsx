...
export default function PlatformSessions() {
  ...
  async function handleConfirmRevoke() {
    ...
    setRevoking(true);
    try {
      await api.post(`/sessions/${sessionToRevoke.id}/revoke/`);
      setMessage(`${sessionToRevoke.username} was signed out.`);
      await load();
    } catch (requestError) {
      setError(requestError.response?.data?.detail || "Unable to revoke this session.");
    } finally {
      setRevoking(false);
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Review active access and revoke sessions that should no longer be trusted.</p>
        </div>
        <button onClick={load} disabled={loading} className="inline-flex items-center justify-center gap-2 px-3 py-2 rounded-md border border-slate-700 text-sm text-slate-300 disabled:opacity-50">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
        <div className="inline-flex self-start p-1 bg-slate-900 border border-slate-800 rounded-md">
          {[["active", "Active"], ["revoked", "Revoked"], ["all", "All"]].map(([value, label]) => (
            <button key={value} onClick={() => setFilter(value)} className={`px-3 py-1.5 rounded text-xs font-medium ${filter === value ? "bg-slate-700 text-white" : "text-slate-500 hover:text-slate-200"}`}>
              {label}
            </button>
          ))}
        </div>
          ...
          <tbody>
            {filtered.map((session) => (
              <tr key={session.id}>
                ...
                <td>
                  <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded text-xs ${session.revoked_at ? "bg-slate-700/50 text-slate-400" : "bg-emerald-400/10 text-emerald-300"}`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${session.revoked_at ? "bg-slate-500" : "bg-emerald-400"}`} />
                    {session.revoked_at ? "Revoked" : "Active"}
                  </span>
                </td>
                <td className="text-right">
                  {!session.revoked_at && (
                    <button title="Force logout" aria-label={`Force ${session.username} to log out`} onClick={() => revoke(session)} className="inline-flex p-2 rounded text-rose-300 hover:bg-rose-500/10">
      ...
      <ConfirmDialog
        ...
        message={`Force ${sessionToRevoke?.username} to sign out? This will immediately terminate their current session.`}
